...
N=256;                               %number of atoms in simulation
...
t_total = 100000;                     % total simulation time (in number of steps at rate t_step)
t = 0;                              % current time (in number of time steps)
t_stats = 100;                       % how often energy and momentum statistics are outputted
...
%Set initial conditions if the simulation is not a restart
if RESTART == 1    
[x,v,F,P,P_virial,T,KE,PE]=initialization_fresh(N,L,pe_cutoff,a,a2,f_cutoff,cutoff,x,epsilon_Ar,sigma_Ar,m,kb,V,t_step,mass_Ar);
else
    %read_data
    x = dlmread('D:\Classes\CMU\Molecular Simulation\HW3\liquid256.txt');       %read in initial positions
    ...
    L(1,1:3)=6.8;                                          %Set simulation domain to basically infinite
    V=L(1,1)*L(1,2)*L(1,3);
...
for t=1:t_total  
    ...
    if rem(t,t_stats)==0
        ...
        props(pIndex,1) = t;
        props(pIndex,2)=T*(epsilon_Ar/kb); 
        props(pIndex,3)=(P*(epsilon_Ar/(sigma_Ar*sigma_Ar*sigma_Ar)))/1000000;
        props(pIndex,4)=(P2*(epsilon_Ar/(sigma_Ar*sigma_Ar*sigma_Ar)))/1000000;
        props(pIndex,5)=V;
        props(pIndex,6)=KE; props(pIndex,7)=PE; props(pIndex,8)=PE+KE;
        props(pIndex,9)=sum(p(:,1));
        props(pIndex,10)=sum(p(:,2));
        props(pIndex,11)=sum(p(:,3));
        pIndex=pIndex+1;
        %write trajectories to file in CHIME format
        str = 'D:\Classes\CMU\Molecular Simulation\HW3\Q1\props\';
        dlmwrite(strcat(str,'x.xyz'),x,'-append','delimiter',' ');
        dlmwrite(strcat(str,'props.dat'),props,'-append');
        t
    end
end

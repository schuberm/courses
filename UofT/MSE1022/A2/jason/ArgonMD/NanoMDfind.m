...
N=10;                               %number of atoms in simulation
...
%[x,v,F,P,P_viral,T,KE,PE]=initialization_fresh(N,L,pe_cutoff,a,a2,f_cutoff,cutoff,x,epsilon_Ar,sigma_Ar,m,kb,V,t_step,mass_Ar);
    ...
    [F,P,P_viral]=force_find(N,L,a2,f_cutoff,a,x,cutoff,V,0);    %find initial forces
...
for t=1:t_total  
    ...
    [F,P,P_viral]=force_find(N,L,a2,f_cutoff,a,x,cutoff,V,T);
    ...
    if rem(t,t_stats)==0
        ...
        props(pIndex,1)=T; props(pIndex,2)=P; props(pIndex,3)=V;
        props(pIndex,4)=KE; props(pIndex,5)=PE; props(pIndex,6)=PE+KE;
        props(pIndex,7)=sum(p(:,1)+p(:,2)+p(:,3));
        props(pIndex,:);
        pIndex=pIndex+1;
        %write trajectories to file in CHIME format
        str = 'D:\Classes\CMU\Molecular Simulation\HW2\Q3\props\';
        dlmwrite(strcat(str,'x.xyz'),N,'-append');
